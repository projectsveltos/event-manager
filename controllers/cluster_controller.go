...
func processCluster(ctx context.Context, c client.Client, cluster client.Object,
	req ctrl.Request, clusterType libsveltosv1beta1.ClusterType, logger logr.Logger) (ctrl.Result, error) {

	if err := c.Get(ctx, req.NamespacedName, cluster); err != nil {
		if apierrors.IsNotFound(err) {
			err = removeEventReportsFromCluster(ctx, c, req.Namespace, req.Name,
				clusterType, map[string]bool{}, logger)
			if err != nil {
	...
	if !cluster.GetDeletionTimestamp().IsZero() {
		...
		clustercache.GetManager().RemoveCluster(req.Namespace, req.Name, clusterType)
		err := removeEventReportsFromCluster(ctx, c, req.Namespace, req.Name,
			clusterType, map[string]bool{}, logger)
		if err != nil {
			return reconcile.Result{}, err
